...
# edge. For example, the 6th row has 6 as the first entry indicating that this row corresponds to the vertex labeled
# 6. The next entry of this row “141,8200” indicates that there is an edge between vertex 6 and vertex 141 that has
# length 8200. The rest of the pairs of this row indicate the other vertices adjacent to vertex 6 and the lengths of
...
# should be 1000,1000,1000,1000,1000,2000,1000,1000,1000,1000. Remember the order of reporting DOES MATTER,
# and the string should be in the same order in which the above ten vertices are given. The string should not contain
...
def main():
    # Read the file, and create a dictionary with the graph and the weights
    graph = {}
    with open("dijkstraData.txt") as f:
        for line in f:
            ...
            graph[int(line[0])] = {}
            for x in line[1:]:
                graph[int(line[0])][int(x.split(",")[0])] = int(x.split(",")[1])
    # print(graph)
    ...
    while len(X) < len(graph):
        # among all edges (v, w) in E with v in X and w not in X, pick the one that minimizes A[v] + l_vw
        min = sys.maxsize
        for x in X:
            for y in graph[x]:
                if y not in X:
                    if A[x] + graph[x][y] < min:
                        min = A[x] + graph[x][y]
                        v = x
                        w = y
        # add w to X
